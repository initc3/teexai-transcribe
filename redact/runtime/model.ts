// Smithers AgentLike backed by z.ai glm-4.6 with forced JSON output. Mirrors pc/model.ts
// but omits temperature (no temp-0) — each reviewer's persona is its system instruction.
...
export function redactAgent(instructions: string) {
  return {
    ...
    async generate(args: any) {
      const body = {
        model: "glm-4.6",
        max_tokens: 2048,
        thinking: { type: "disabled" },  // the passes ARE the reasoning; hidden CoT just starves the JSON output
